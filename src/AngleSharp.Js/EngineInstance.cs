namespace AngleSharp.Js
{
    ...
    sealed class EngineInstance
    {
        ...
        private ObjectInstance CreatePrototype(Type type)
        {
            ...
            {
                foreach (var name in new[] { "entries", "forEach", "keys", "values" })
                {
                    prototype.FastSetProperty(name, new PropertyDescriptor(
                        _engine.Intrinsics.Array.PrototypeObject.Get(name), true, false, true));
                }
            }
